...
fn exception(frame: &mut ExceptionFrame) {
    ...
    match ec {
        ...
        EC_DABT | EC_DABTL => {
            kprintln!("Data abort with ISS: {}", frame.esr & 0xFFFF);
        ...
        EC_BRK => {
            kprintln!("Software break point");
            loop {}
        }
        EC_SVC => {
            let svc_number = (frame.esr & 0xFFFF) as u16;
            match svc_number {
                0x1337 => { kprintln!("EC_SVC from userspace: {svc_number}!");}
                _ => {kprintln!("EC_SVC exception with number {svc_number}!");}
            }
        }
